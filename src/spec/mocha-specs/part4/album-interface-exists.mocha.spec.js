...
describe('Album Interface', function() {
  it(`should exist @album-interface-exists`, function () {
    assert(fs.existsSync(__dirname + '/../../../app/models/album.ts'), "The Album interface hasn't been created yet.");

    let file;
    try {
      file = fs.readFileSync(__dirname + '/../../../app/models/album.ts').toString();
    } catch (e) {
